...
func main() {
	...
	RemainedTxsThreshold := 72000
	for iteration := 0; iteration < 100; iteration++ {

		// 读取address location文件
		FromAddLoc_tocsv := test.ReadAddLoc("from_address_location_zipf_1.csv")
		...
		if RemainedTxsLastEpoch < RemainedTxsThreshold {
			...
			test.WriteAddLocToCSV(FromAddLoc_tocsv, "FromAddLoc_all_mig_Mar31_10min_zipf1.csv")
			test.WriteAddLocToCSV(ToAddLoc_tocsv, "ToAddLoc_all_mig_Mar31_10min_zipf1.csv")

			// 写入csv文件
			f, err := os.Create("remainedtx_mig_top_noerror_Mar31_10min_zipf1.csv") //创建文件
			if err != nil {
				panic(err)
			}
			defer f.Close()
			...
			f1, err := os.Create("load_mig_top_noerror_Mar31_10min_zipf1.csv") //创建文件
			if err != nil {
				panic(err)
			}
			defer f1.Close()
