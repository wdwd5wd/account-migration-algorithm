...
func main() {
	...
	for i := 0; i < len(UnixTime); i++ {
		...
		if UnixTime[i] < timeCheckPoint+int64(timeSlot) {
			...
			fmt.Println("finish epoch:", CheckPoint)
	...
	// 写入csv文件
	f, err := os.Create("remainedtx_with_mig_top_witherror.csv") //创建文件
	if err != nil {
		panic(err)
	}
	defer f.Close()
	...
	f1, err := os.Create("load_with_mig_top_witherror.csv") //创建文件
	if err != nil {
		panic(err)
	}
	defer f1.Close()
